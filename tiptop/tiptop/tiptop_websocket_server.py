...
Usage:
    pixi run tiptop-server --port 8765
...
class TiptopPlanningServer:
    ...
    async def _run_pipeline(self, obs: dict) -> dict:
        ...
        try:
            ...
            q_init = obs["q_init"]
            raw_views = obs.get("observations")
                ...
                raw_views = [
                    {
                        ...
                        "world_from_cam": obs["world_from_cam"],
                    }
                ]

            decoded_observations = []
            for raw_view in raw_views:
                rgb = raw_view["rgb"].astype(np.uint8)
                depth = raw_view["depth"].copy().astype(np.float32)
                K = raw_view["intrinsics"].astype(np.float32)
                world_from_cam = raw_view["world_from_cam"].astype(np.float32)
                ...
                decoded_observations.append(Observation(frame=frame, world_from_cam=world_from_cam, q_init=q_init))

            observation = decoded_observations[0]
            additional_observations = decoded_observations[1:]
            rgb = observation.frame.rgb
            ...
            _log.info(
                f"Processing {len(decoded_observations)} view(s): RGB shape={rgb.shape}, "
                f"depth shape={observation.frame.depth.shape}, task='{task_instruction}'"
            )
