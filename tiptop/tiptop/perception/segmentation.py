...
def segment_table_with_ransac(
    ...
    # Adjust height position so the top of the box aligns with the detected surface.
    # The collision cuboid must match the visible tabletop; otherwise planning can
    # accept grasps that already penetrate the real table.
    extents = table_box.extents
    table_center = table_box.center_mass
    height_offset = surface_z - (table_center[2] + extents[2] / 2)
...
def augment_with_base_projections(
    ...
    """Augment a point cloud by appending a copy of each point flattened near the object's support plane.
    ...
    """
    # Use a low percentile instead of absolute min to avoid a few noisy low points
    # dragging the support plane far below the true object base.
    base_z = float(np.percentile(points[:, 2], 5.0))

    projected_points = points.copy()
    projected_points[:, 2] = base_z
...
def segment_pointcloud_by_masks(
    ...
    erode_pixels: int = 0,
    support_cutoff_fallback_slack_m: float = 0.0,
    ...
    Args:
        ...
        support_cutoff_fallback_slack_m:
            If no points remain above `max_z`, lower the support cutoff by this slack
            before falling back to highest-z points. Helps recover thin objects when table
            plane estimation is slightly above the object base.
    ...
    for mask_2d, bbox in zip(masks_2d, bboxes):
        ...
        if z_mask.sum() == 0 and support_cutoff_fallback_slack_m > 0.0:
            relaxed_cutoff = max_z - support_cutoff_fallback_slack_m
            z_mask = xyz_obj[..., 2] > relaxed_cutoff
            if z_mask.sum() > 0:
                _log.warning(
                    f"{label}: no points above support cutoff z={max_z:.3f}; "
                    f"using relaxed cutoff z={relaxed_cutoff:.3f} with {int(z_mask.sum())} points"
                )
        if z_mask.sum() == 0:
            # Thin or noisy tabletop objects can end up numerically flush with the detected
            # support plane. Keep the highest mask points so downstream multiview fusion and
            # grasping can still proceed instead of crashing on an empty point set.
            fallback_count = int(np.clip(len(xyz_obj) * 0.01, 64, 384))
            fallback_count = min(len(xyz_obj), fallback_count)
            top_idxs = np.argpartition(xyz_obj[:, 2], -fallback_count)[-fallback_count:]
            z_mask = np.zeros(len(xyz_obj), dtype=bool)
            z_mask[top_idxs] = True
            _log.warning(
                f"{label}: no mask points above support cutoff z={max_z:.3f}; "
                f"falling back to top {fallback_count} highest points"
            )
