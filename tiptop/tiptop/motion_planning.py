...
def _build_motion_collision_cache(world_cfg: WorldConfig) -> dict[str, int]:
    """Build a generous cuRobo collision cache so later update_world() calls do not overflow."""
    cuboids = getattr(world_cfg, "cuboid", None) or []
    meshes = getattr(world_cfg, "mesh", None) or []
    return {
        "obb": max(128, len(cuboids) + 64),
        "mesh": max(32, len(meshes) + 16),
    }
...
def get_motion_gen(
    ...
    with patch_log_level("curobo", logging.ERROR):
        collision_cache = _build_motion_collision_cache(world_cfg)
