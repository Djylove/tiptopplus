...
from tiptop.config import resolve_sibling_repo_path, resolve_workspace_root
...
def prompt_hand_depth_source(current_value: str) -> str:
    """Prompt user to select the hand-camera depth source."""
    aliases = {
    ...
    default = aliases.get(str(current_value).strip().lower(), "foundation_stereo")
    while True:
        user_input = input(
            "Hand camera depth source (foundation_stereo/sensor, D435 usually uses foundation_stereo) "
            f"[{default}]: "
        ).strip().lower()
        if not user_input:
            return default
        normalized = aliases.get(user_input)
        if normalized is not None:
            return normalized
        print("  ⚠️  Please enter 'foundation_stereo' or 'sensor'.")
...
def entrypoint():
    ...
    workspace_root = resolve_workspace_root(cfg=config)
    foundation_stereo_root = resolve_sibling_repo_path(
        "Fast-FoundationStereo",
        config_path="perception.foundation_stereo.project_root",
        cfg=config,
    )

    print(f"\nResolved workspace root: {workspace_root}")
    print("Override precedence:")
    print("  1. CLI/config.profile or TIPTOP_CONFIG_PROFILE")
    print("  2. environment variables such as TIPTOP_WORKSPACE_ROOT")
    print("  3. values saved in tiptop/config/tiptop.yml")
    print("Validated local baseline:")
    print(f"  workspace.root -> {workspace_root}")
    print(f"  foundation_stereo.project_root -> {foundation_stereo_root}")
    if os.environ.get("TIPTOP_CONFIG_PROFILE"):
        print(f"Active TIPTOP_CONFIG_PROFILE: {os.environ['TIPTOP_CONFIG_PROFILE']}")
    ...
    config["perception"]["hand_depth_source"] = prompt_hand_depth_source(
        config["perception"].get("hand_depth_source", "foundation_stereo")
    )
    config["perception"]["foundation_stereo"]["url"] = prompt_with_default(
        "FoundationStereo URL", config["perception"]["foundation_stereo"]["url"]
    )
    config["perception"]["foundation_stereo"]["project_root"] = prompt_with_default(
        "Local Fast-FoundationStereo project root",
        str(foundation_stereo_root),
    )
