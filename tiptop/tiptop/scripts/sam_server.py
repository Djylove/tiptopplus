...
def _import_legacy_sam2_server_backend():
    try:
        from sam2.build_sam import build_sam2
        from sam2.sam2_image_predictor import SAM2ImagePredictor
    except ModuleNotFoundError as exc:
        missing_name = getattr(exc, "name", "") or ""
        if missing_name.startswith("sam2"):
            raise RuntimeError(
                "The `sam-server` command is a legacy SAM2 compatibility path. "
                "Install it explicitly with `pip install -e \".[sam-server]\"` before starting this server."
            ) from exc
        raise
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        build_sam2, SAM2ImagePredictor = _import_legacy_sam2_server_backend()
