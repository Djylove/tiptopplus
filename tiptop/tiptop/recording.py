...
def _get_git_root() -> Path | None:
    """Return the repository root, or None when this workspace is not a git repo."""
    try:
...
def _collect_git_info() -> dict:
    ...
    root = _get_git_root()
    if root is None:
        _log.info("Git metadata unavailable for this workspace; saving null git fields in metadata")
    ...
    try:
        ...
        _log.info("Git metadata collection failed; continuing without commit info")
        return {"commit": None, "dirty": None, "porcelain": None}
...
def _get_git_diff() -> str | None:
    ...
    try:
        ...
        _log.info("Git diff unavailable; continuing without git.diff artifact")
        return None
