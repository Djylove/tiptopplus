...
If you are bringing up a local GPU workstation from scratch or troubleshooting a source-archive build, see the more detailed [development build notes](development-build.md).
...
- A configured VLM provider: `GOOGLE_API_KEY` for Gemini, `TIPTOP_VLM_PROVIDER=codex` for a logged-in local Codex CLI, or `TIPTOP_VLM_PROVIDER=openai` plus `OPENAI_API_KEY` for OpenAI API models. If `TIPTOP_VLM_PROVIDER` is unset and `GOOGLE_API_KEY` is missing, TiPToP will auto-fallback to a logged-in local Codex CLI when available.
