...
M2T2 and a FoundationStereo-compatible depth server can be installed on the same machine as TiPToP. However, if you're using the DROID GPU laptop and have a separate GPU workstation available, we recommend installing M2T2 and the depth server on that workstation to improve inference time.
...
- **Disk space:** Approximately 50GB total if you also install the original FoundationStereo pixi environment (17GB for TiPToP/cuRobo/cuTAMP + 13GB for M2T2 + 20GB for FoundationStereo). If you instead reuse the validated local `/home/user/Fast-FoundationStereo` replacement described below, the extra requirement is only that checkout's own `.venv` and weights. Check available space with `df -h`.
...
```{note}
On this workstation we validated a drop-in replacement at `/home/user/Fast-FoundationStereo`. It can live outside `$TIPTOP_DIR` because TiPToP only depends on the HTTP API exposed by the depth server.
```
...
TiPToP uses a FoundationStereo-compatible HTTP server to generate depth maps from stereo camera images. On this workstation, the validated and preferred path is to reuse the existing local `/home/user/Fast-FoundationStereo` checkout and run its TiPToP-compatible server.

```{important}
- Reusing `/home/user/Fast-FoundationStereo` avoids the original FoundationStereo `flash-attn` build path and separate checkpoint download step.
- The local Fast-FoundationStereo server keeps the same TiPToP-facing API: `GET /health` and `POST /infer` returning a compressed `npz` with `depth`.
...
If you're using the DROID GPU laptop, we recommend running this server on a separate GPU workstation if available to improve inference time. Otherwise, run it on the same machine where you installed TiPToP.

```bash
export FAST_FOUNDATION_STEREO_DIR=/home/user/Fast-FoundationStereo
cd $FAST_FOUNDATION_STEREO_DIR

# Optional: override the default serialized model path.
export FAST_FOUNDATION_STEREO_MODEL=$FAST_FOUNDATION_STEREO_DIR/weights/23-36-37/model_best_bp2_serialize.pth

# Start the TiPToP-compatible depth server.
.venv/bin/python scripts/tiptop_server.py --host 127.0.0.1 --port 1234
```

```{note}
`scripts/tiptop_server.py` automatically loads `weights/23-36-37/model_best_bp2_serialize.pth` when `FAST_FOUNDATION_STEREO_MODEL` is not set. If you want the server reachable from another machine, replace `127.0.0.1` with `0.0.0.0` and point `tiptop/config/tiptop.yml` at `http://<host>:1234`.
```

Validate the server from another terminal:

```bash
curl -fsS http://127.0.0.1:1234/health
```

The response should report `"status":"healthy"` and the resolved model path. TiPToP's existing client in `tiptop/perception/foundation_stereo.py` works unchanged against this server.

If you explicitly need the original FoundationStereo fork and its pixi environment, use the fallback notes in [Development Build Notes](development-build.md). Those notes also cover the `flash-attn` freeze diagnosis and the checkpoint download workarounds.
